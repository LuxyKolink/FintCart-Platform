...
- [ ] T091 [US3] Ejecutar por `calculator_id` en `Compute`, conservando `calc_type` como camino de compatibilidad y validando rangos de entrada (FR-044), en `services/simulator/src/grpc/service.rs` — **BLOQUEADA por una decisión de esquema, no por trabajo pendiente.** `simulations.calc_type` es `NOT NULL` con un `CHECK` de cinco valores (`20260727120000_init_simulator.up.sql:46`), y una simulación de una calculadora de USUARIO no tiene ninguno de esos cinco: hoy no existe forma de insertarla. T020 conservó la columna «para no invalidar el historial existente» sin advertir este caso. Hasta resolverlo, `Compute` **rechaza** `calculator_id` con `Unimplemented` nombrando esta tarea en vez de seguir por `calc_type`: un cliente que pide una calculadora concreta recibía el resultado de otra sin que nada se lo dijera. La decisión (columna anulable + «o tipo nativo o referencia a calculadora») toca también a T103 y a `ListHistory`, porque `ListCalculatorsResponse` ya devuelve el estado como texto pero el historial devuelve el enum
