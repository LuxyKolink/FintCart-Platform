...
- [ ] T128 [US6] Extracción y validación de las referencias a imágenes del documento al guardar, en `services/learning/src/articles/` — **la EXTRACCIÓN está hecha y probada** (`extractBodyDocReferences` en `body-doc.validator.ts`, con sus casos: sin repetir, en orden de aparición, y encontrando las referencias anidadas dentro de listas). **Falta el gancho**: recorrer el árbol al guardar y comprobar contra `article_images` que cada `image_id` existe. No se hace todavía porque el documento que llega hoy se deriva del texto (T124), así que un documento con referencias solo puede venir de la migración, y **validar referencias de contenido ya escrito crearía un fallo que nadie puede arreglar**: el artículo ya está publicado y su imagen es anterior a la tabla. El gancho entra con el editor (T131/T132), que es cuando el documento empieza a llegar del cliente
