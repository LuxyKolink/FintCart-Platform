...
- [ ] T091 [US3] Ejecutar por `calculator_id` en `Compute`, conservando `calc_type` como camino de compatibilidad y validando rangos de entrada (FR-044), en `services/simulator/src/grpc/service.rs` — **BLOQUEADA por una decisión de esquema, no por trabajo pendiente.** `simulations.calc_type` es `NOT NULL` con un `CHECK` de cinco valores (`20260727120000_init_simulator.up.sql:46`), y una simulación de una calculadora de USUARIO no tiene ninguno de esos cinco: hoy no existe forma de insertarla. T020 conservó la columna «para no invalidar el historial existente» sin advertir este caso. Hasta resolverlo, `Compute` **rechaza** `calculator_id` con `Unimplemented` nombrando esta tarea en vez de seguir por `calc_type`: un cliente que pide una calculadora concreta recibía el resultado de otra sin que nada se lo dijera. **RESUELTA por D-26**: la columna admite un sexto valor, `'usuario'`, y el enum un sexto miembro, `CALC_TYPE_USUARIO = 6`; la columna sigue `NOT NULL`, así que no riega `SimulationRow` ni el historial. **El vocabulario YA ESTÁ HECHO** (commits `210b4e9` de contrato con los stubs regenerados, y `4d21e15` de lógica): la migración `20260902124500` amplía el `CHECK`, el enum del contrato tiene `CALC_TYPE_USUARIO = 6`, `dispatch::stored_to_proto` traduce la columna al enum y `Kind::from_proto` rechaza `usuario` en una petición. **`Kind` NO ganó una variante**: es la lista de calculadoras NATIVAS y `compute` elige con él una función nativa, así que un brazo suyo para «usuario» sería inalcanzable; lo único que los dos caminos comparten es el nombre del valor. Queda por hacer, y por eso la tarea sigue abierta, la ejecución: resolver `calculator_id` a una definición, ejecutarla y persistir con `calc_type = 'usuario'`. **Y ahí aparece una dependencia que este ledger no tenía anotada: T091 necesita T101.** `Definition::run` recibe los indicadores vigentes, y hoy no existe ninguna consulta que los resuelva por fecha —`known_indicators()` devuelve solo NOMBRES—, así que `gmf` no se puede ejecutar por `calculator_id` hasta que exista la resolución por vigencia de `repo/indicators.rs`. Las dos van juntas o T091 entrega una ejecución que falla justo con la calculadora que depende de `@UVT` **Ojo con `domain::dispatch::Kind`**: hoy es el vocabulario de los cinco tipos NATIVOS y `compute` lo usa para elegir la función nativa, mientras que la columna pasa a tener un valor que no es nativo. Extender `Kind` con una variante que `compute` rechace es una opción; separar «tipo de simulación» de «calculadora nativa» es la otra, y es la que el historial necesita, porque `ListHistory` mapea el valor almacenado al enum del contrato y no despacha nada. La decisión toca también a T103
...
- [ ] T103 [US4] Persistir `indicators_snapshot`, `calculator_id` y `calculator_version` en cada simulación, en `services/simulator/src/repo/simulations.rs` (FR-050, FR-058) — **depende de D-26**, junto con T091: las dos escriben simulaciones de calculadoras de usuario y las dos necesitan el sexto valor de `calc_type`. Se resuelven en la misma unidad, o la decisión se toma dos veces. `indicators_snapshot` no está condicionado por D-26: es un mapa `{nombre: valor}` y FR-058 lo exige en toda simulación, venga del camino que venga
