...
- [ ] T006 Regenerar y versionar los stubs de los cinco stacks con `contracts/generate.sh`, **en un commit propio sin cambios de lógica** (Constitución §Definición de Contratos) — **parcial y por partes, a propósito**: los stubs se regeneran cuando se aplica cada delta, no cuando se aplican todos, porque los servicios tienen que compilar mientras el resto de la enmienda avanza. Hasta ahora están regenerados los de Aprendizaje (Go en api-gateway, auth-server, orchestrator, users y audit; TypeScript en Aprendizaje, Notificación y Frontend) tras añadir `body_doc`, y el de Rust quedó intacto porque `simulator.proto` no cambió — comprobado, no supuesto. El delta de Usuarios (T003) todavía no se ha aplicado, así que el guion no puede correr completo. Además, hoy **no puede correr completo en esta máquina**: comprueba las herramientas antes de empezar y `cargo` no está instalado, así que se replicaron sus mismos comandos para Go y TypeScript en lugar de saltarse el orden que él fija
...
- [X] T016 [P] Migración emparejada que añade `article_versions.body_doc JSONB` y lo puebla envolviendo cada `body` en párrafos; **`body` NO se elimina aquí** (research D-14), en `services/learning/migrations/` — **APLICADA y verificada contra PostgreSQL 16 con las 79 versiones reales**: `79 de 79` con raíz `doc`, `89` párrafos y **0 pérdidas de texto** (la guarda del final de la migración lo comprueba y aborta si alguna versión pierde texto, comparando **sin espacios**, porque las líneas en blanco son separadores y no contenido). La reversión se probó sobre una **copia** de la base, no sobre la base en uso, y deja el estado exactamente como estaba. **Dos errores propios, corregidos y anotados** — uno por escribir mal la migración y otro por intentar arreglarlo a mano: (1) la columna nació `NOT NULL`, y quien crea versiones hoy (`publishing.repository.ts`, cinco columnas, ninguna `body_doc`) **no la rellena**, así que crear un borrador habría empezado a fallar al aplicar la migración; se comprobó leyendo el `INSERT`, se dejó anulable con el `NOT NULL` reservado a T135 —cuando `body` desaparezca— y se probó después el `INSERT` real del servicio contra la base. (2) Para revertir borré a mano la fila de `schema_migrations`: la tabla quedó vacía, `golang-migrate` volvió a empezar **desde la primera migración**, esta falló y dejó la base marcada como sucia. Es la misma clase de atajo que este libro ya tenía anotada, ahora con un motor que no perdona el atajo — y el precio fue menor solo porque se vio en el registro
- [X] T017 [P] Migración emparejada `article_images` (PK = SHA-256 hex, `BYTEA` con `STORAGE EXTERNAL`, CHECK de mime y de tope de 2 MB) en `services/learning/migrations/` — **APLICADA y verificada**, y **cada CHECK probado con una fila que tenía que rechazar**, porque una restricción que nunca ha rechazado nada es una esperanza: identificador que no es SHA-256 ✗, mime no admitido ✗, `byte_size` que no cuadra con los bytes ✗, `byte_size` en cero ✗, ancho en cero ✗, más de 2 MB ✗, fila válida ✓; y subir dos veces el mismo contenido choca con la clave primaria, que es la deduplicación por contenido funcionando. Se añadió `article_images_size_matches_bytes` (`byte_size = octet_length(bytes)`), que `data-model.md` §1.4 no pedía: sin esa igualdad, el tope de 2 MB sería una promesa sobre una columna que nadie compara con el contenido real y bastaría declarar `byte_size = 1` para guardar lo que fuera. `byte_size` se conserva —leer el tamaño sin traer el `BYTEA` es lo que evita una consulta de listado pesada— pero ahora es un dato derivado **comprobado**

> **Lo que la verificación de esta fase destapó, y que no es de la fase.** Al ejecutar la
> batería completa de extremo a extremo para comprobar T016 aparecieron dos cosas que no tienen
> que ver con el esquema y que están en `findings.md`: el error interno del Gateway **no dice qué
> dependencia falló** (cuesta 20 minutos de depuración lo que podría costar una línea), y con la
> suite completa encadenada aparecen fallos de **5–6 s** repartidos entre tres servicios que en
> aislado no ocurren. Lo segundo es contención de la máquina, no del código — se descartaron
> índices, colas y consultas largas midiendo, y el plazo del Gateway es de 20 s —, pero significa
> algo que conviene saber: **hoy la batería completa no es repetible dos veces seguidas en este
> equipo**, así que antes de una demostración conviene levantar la pila de cero y correrla una vez.
...
- [X] T097 [P] [US3] Constructor visual de calculadoras con validación en vivo contra `/calculators/validate`, y ejecutor de resultados, en `frontend/src/app/features/calculators/{builder,runner}/` — montos, tasas y resultados con `decimal.js` reutilizando `frontend/src/app/shared/decimal-str.ts` y `features/simulators/decimal-validators.ts`; **prohibido `number` nativo** (Principio VIII / FR-048) — **HECHA (2026-09-17)**: `features/calculators/builder/` con las tres listas repetibles (datos, reglas y resultados), la comprobación en vivo con 400 ms de retardo contra `POST /calculators/validate` —los problemas llegan TODOS y cada uno con su `location`, que es lo que resalta el campo exacto: `inputs[1].key` → `inputs.1.key`—, y el guardado que crea o versiona. Tres decisiones que quedan escritas en el código: **el veredicto lo da el servidor** (aquí no hay un segundo analizador; el `valid` es suyo, y un fallo de red NO declara la definición inválida), **guardar no publica** (editar una publicada crea una versión que sigue privada hasta la aprobación, y la pantalla lo dice), y **al crear la ruta pasa a la de edición** para que recargar no cree OTRA calculadora. Las cotas viajan como cadenas y la escala con `parseInt` —un recuento de decimales no es una cifra—, que es lo que deja la barrera de FR-068 sin excepciones. Tres defectos reales aparecieron al hacerla, los tres en `findings.md`: el contrato manda `min_value`/`max_value`/`default_value` y el ejecutor leía `min`/`max`/`default` (así que **no comprobaba ninguna cota** ni rellenaba ningún valor por defecto), el ejecutor mandaba al servidor un valor que su propio formulario ya sabía fuera de rango, y **el motivo de un rechazo no llegaba al usuario**: el orquestador aplanaba la cadena de la saga y el borde la sustituía por «petición inválida», de modo que la regla `monto > 1000` con el mensaje «El monto tiene que superar 1000» respondía «petición inválida» (FR-044/FR-045). Verificado de extremo a extremo con `e2e/constructor-calculadora.spec.ts` —se escribe, el servidor señala el error, se corrige, se guarda, se ejecuta y la cota declarada rechaza un valor fuera de rango— más 284 pruebas unitarias y 56/56 de la batería
- [X] T098 [US3] Eliminar `services/simulator/src/calculators/` y redirigir el despacho a las definiciones semilla — **PARCIAL: la mitad que no depende del sembrado está hecha (`2798f01`), y la otra mitad está ORDENADA DESPUÉS de D-28 por una razón que la tarea no decía.** Hecho: `annuity` mudado a `src/domain/annuity.rs` y sus tres multiplicaciones pasadas a `checked_mul` (D-27). La mudanza no era cosmética —`domain/formula/functions.rs` importaba `crate::calculators::annuity` para implementar `cuota`, `vf_serie` y `tasa_periodica`, que son funciones del LENGUAJE, así que la capa de dominio dependía del código nativo que esta tarea viene a retirar: tal como estaba, borrar `src/calculators/` habría dejado al motor sin su aritmética—. La corrección de D-27 se supo que había surtido efecto porque la suite de regresión ACOTABA esos casos en vez de ignorarlos: en cuanto el código nativo dejó de abortar, la prueba falló sola. **Lo que falta**: redirigir el camino de compatibilidad a las definiciones semilla y borrar `ahorro.rs`, `credito.rs`, `presupuesto.rs` e `inversion.rs`. Sus entradas coinciden EXACTAMENTE con las de sus semillas —comprobado—, así que esa redirección no cambia lo que acepta ningún cliente; lo único que cambia son los textos de error, que los de la semilla son deliberadamente mejores, y los casos de desbordamiento, que dejan de ser un 500 por pánico. **`colombia.rs` NO se borra con ellos** (D-30). **Y el bloqueo, que es de orden y no de trabajo**: el despacho redirigido resuelve la definición DESDE LA BASE, así que sobre una base migrada y sin sembrar el Simulador se queda sin ninguna calculadora — la tarea quita el camino nativo y con él la única forma de calcular. En desarrollo `dev/seed` corre siempre; en producción ese paso **no existía** —D-28 lo dejó anotado como pendiente—. **Ya está escrito: `deploy/vps/seed`** (2026-09-16), junto con el hueco que apareció al escribirlo —el binario de siembra no estaba en la imagen de producción, porque el Dockerfile compilaba el árbol entero pero solo copiaba `fintcart-simulator`—. Con eso el orden queda satisfecho y T098 puede hacerse; hacerlo ANTES habría dejado la primera migración de producción sin sembrar con el simulador entero caído — **HECHA (2026-09-17)**: el camino de compatibilidad por `calc_type` ya no ejecuta código nativo de las cuatro que coinciden con su semilla —`ahorro`, `credito`, `presupuesto`, `inversion`— sino que **lee la definición semilla y la ejecuta**, la misma consulta y el mismo motor que el camino preferente por `calculator_id`. El servicio dejó de compilar esos cuatro módulos: viven en `tests/nativo/`, donde `tests/seed_regression.rs` los usa como ORÁCULO, y no se sustituyó la comparación por valores congelados a propósito —ver la cabecera de ese módulo—. `colombia.rs` se queda con `compute_colombia`, que ya no se despacha por `Kind` y se nombra para que se vea que es la excepción. `Error::MissingSeed` nace aquí: sobre una base sin sembrar una ejecución por `calc_type` dice qué semilla le falta en lugar de calcular por un camino que ya no existe (D-30, `deploy/vps/seed`). Verificado: 264 pruebas unitarias, 28 contra PostgreSQL real y **55/55 de extremo a extremo**, que es donde se ve que las calculadoras clásicas devuelven las mismas cifras por el motor de fórmulas
...
- [X] T099 [P] [US4] Prueba de que una vigencia solapada se rechaza **en la base** por la restricción de exclusión, no solo en la aplicación, en `services/simulator/tests/indicators_db.rs` — **el archivo es `indicators_db.rs`** y la prueba se llama `dos_vigencias_solapadas_las_rechaza_la_base`. La comprobación que importa es el `INSERT` DIRECTO, saltándose el repositorio entero: el repositorio también comprueba el solapamiento, y esa comprobación NO es la garantía —entre la lectura y la escritura cabe otro administrador cargando el mismo año—. Se afirma el código `23P01` de la violación de exclusión y su traducción a `AlreadyExists`, que es lo que hace que dos administradores simultáneos reciban un 409 y no un 500 reintentable. Se añaden dos casos alrededor que son los que distinguen un `EXCLUDE` correcto de una comprobación mal escrita: vigencias CONTIGUAS que conviven (el extremo superior es exclusivo: `[2026-01-01, 2027-01-01)` y `[2027-01-01, 2028-01-01)` no se pisan) y que nada de lo rechazado quede guardado
...
- [X] T104 [US4] `UpsertIndicator`, `ListIndicators` y `GetIndicatorCalendarStatus` en `services/simulator/src/grpc/service.rs` — **los tres tienen cuerpo y ya no devuelven `pending(...)`**, así que el motor de fórmulas queda completo: las fórmulas ya resolvían `@NOMBRE` (T101/T102), las simulaciones ya guardaban su snapshot (T103) y ahora se puede cargar el valor que se resuelve. Cuatro decisiones que no eran obvias: (1) **el nombre no se puede cambiar** —es el vínculo con las fórmulas publicadas que lo referencian, y renombrar es crear otro indicador—; (2) **editar SÍ modifica la fila**, al contrario que en las calculadoras, porque un indicador es una cifra oficial que se transcribe y corregir una errata no crea una versión de la ley (lo que lo hace seguro es el snapshot); (3) **«sin vigencia» se calcula sobre los nombres que YA están en la tabla** —de un indicador que nadie ha cargado no se puede decir que le falte vigencia, y enumerar los que el contexto colombiano podría necesitar sería una lista que nadie mantiene—; (4) **no se avisa de una vigencia que vence si el sucesor ya está cargado**: la que termina el 31 de diciembre no deja hueco si el año siguiente está en la tabla, y una alerta que se dispara sin que haya nada que hacer es la que enseña a no leerlas. Además: `23P01` se traduce en el único embudo por el que entra cualquier error del driver (`Error::from_sqlx`) y no en el repositorio, y `valid_to` vacío significa SIN fecha de fin (el contrato no puede expresar `upper_inf`, que el esquema sí admite). **Tres defectos reales encontrados al escribir las pruebas**: `Error::Decimal` perdía el motivo y el administrador leía «valor decimal no válido» sobre una cifra con siete decimales; `describe_decimal_error` existía TRES veces con textos que ya habían divergido; y `chrono` acepta `2026-1-1` con el formato `%Y-%m-%d`, así que la primera versión del parseo daba por buena una fecha que el contrato no declara. 21 pruebas de contrato (`tests/indicators.rs`) + 13 contra PostgreSQL 16 (`tests/indicators_db.rs`)
- [X] T105 [US4] Barrido periódico del calendario de indicadores y publicación de `indicator.calendar_alert` desde `services/orchestrator/internal/server/sweeper.go` — **el Simulador no es productor** (Principio V, research D-23), así que el aviso se publica desde el Orquestador, que es el único que puede y el que ya tiene el patrón del outbox. La decisión que gobierna el diseño es la IDEMPOTENCIA: el barrido vuelve a pasar cada pocos minutos y el estado que consulta es el mismo mientras dure el problema, así que publicar sin más produciría un correo por vuelta —con el intervalo del compose de desarrollo, doscientos ochenta y ocho al día sobre el mismo dato—, y una alerta que se repite sin que nada haya cambiado es la que enseña a ignorarla. El identificador del evento es **determinista**: `(tipo, clase, indicador, fecha)` por SHA-1 en el espacio de nombres de UUID, de modo que dos barridos del mismo día producen el mismo `event_id` y el `ON CONFLICT (id) DO NOTHING` del outbox descarta el segundo; al día siguiente cambia, y el problema se recuerda sin repetirse. Otras decisiones: el buzón que recibe el aviso es **configuración del despliegue** (`INDICATOR_ALERT_EMAIL`) y no la cuenta con la que alguien entró a cargar las cifras —el Orquestador no conoce a los administradores ni tiene por qué—; sin buzón el barrido **sigue mirando** y lo deja en el log en lugar de tumbar el proceso por una dirección que falta; y un `Unimplemented` del Simulador se distingue de una caída, porque significa otra cosa (una versión desplegada sin los RPC de indicadores). **`indicator.updated` NO se implementa y queda escrito por qué**: no existe productor posible —el Simulador no puede publicar y el alta del administrador no pasa por ninguna saga—, así que publicarlo sería anunciar un evento que nunca ocurre. 11 pruebas del barrido, con un doble del outbox que imita la clave primaria —uno que no dedujera convertiría en verde una prueba sobre la idempotencia—, y verificación de punta a punta: con el barrido a 5 s, el primer paso encoló 1 aviso y los cinco siguientes reportaron 0 nuevos y 1 ya encolado, y **llegó UN solo correo**
- [X] T106 [P] [US4] Plantilla de correo `indicator_calendar_alert` en `services/notification/src/email/templates.ts` con idempotencia por `event_id` — **la idempotencia no la implementa la plantilla, la hereda**: el `event_id` determinista que calcula el barrido entra en `notification_events_queue`, cuya columna `event_id` es `UNIQUE` desde el esquema inicial, y `mapping.ts` ya rechaza un sobre sin él. Lo que la plantilla decide es el TEXTO, y son dos textos distintos: `sin_vigencia` es un problema ACTIVO —las calculadoras que lo referencian están calculando con el dato del año anterior— y `por_vencer` es un aviso anticipado de treinta días. El mismo texto para los dos haría parecer urgente lo que no lo es y dejaría sin urgencia lo que sí; el ASUNTO también los distingue, porque es lo único que se ve en la bandeja de entrada. **El correo no lleva cifras**: quien lo recibe es quien las teclea, y una cifra dentro de un correo invita a copiarla de ahí, que es como se cargan datos equivocados. Una clase de aviso desconocida LANZA (`TemplateError`) en lugar de caer en un texto genérico: entregar algo vago lo contaría como enviado y nadie se enteraría de que el aviso no se entendió. 5 pruebas nuevas y el `CHECK` de plantillas ampliado con su migración
- [X] T107 [US4] Rutas `/admin/indicators[/{id}]`, `/admin/indicators/status` e `/indicators/current` en `services/api-gateway/internal/handler/routes.go`, con 409 para solapamiento — **cinco rutas**: las cuatro de administración con `RequireRole(RoleAdministrator)` y `/indicators/current` en el grupo autenticado, porque la advertencia de FR-062 la ve quien va a ejecutar una calculadora y no solo un administrador. `current` devuelve las dos listas juntas —valores vigentes y nombres sin vigencia— porque quien las usa necesita ambas para decidir si advierte: con dos peticiones, una calculadora que depende de `@UVT` podría ejecutarse con la advertencia sin resolver si la segunda llegara tarde. **El 409 lleva las vigencias que chocan**: el mapeo genérico convertiría el solapamiento en «conflicto con el estado actual», y quien carga el UVT de 2027 necesita ver cuál de las dos cifras sobra; como el mensaje del servicio no cruza la frontera gRPC (política de `httpFromGRPC`), el borde consulta las vigencias de ese nombre y las devuelve en el cuerpo —el mismo patrón que `deactivateCategory` con `published_count`—. La fecha de `current` la pone el servidor **en UTC**, la misma zona con la que el Simulador resuelve al ejecutar: con otra, en el cambio de año el borde diría que todo está en vigor mientras el cálculo no encuentra el valor nuevo. 8 pruebas del borde y el recorrido completo por HTTP real (alta 201 con `49799.00` → `49799`, repetir 409 con las DOS vigencias, corregir 200 con el mismo identificador, renombrar 400, `/indicators/current` con un usuario final 200, la misma ruta con ese usuario 403, y el calendario reportando un indicador sin vigencia). De paso nace **`dev/token <correo> [rol...]`**: `quickstart.md` escribía `$ADMIN_TOKEN` sin decir de dónde sale, y montarlo a mano son seis pasos de los que dos fallan en silencio —el rol no concedido da 403 y un `code_challenge` mal calculado da un `invalid_grant` que no menciona PKCE—
- [X] T108 [P] [US4] Pantalla de carga anual de indicadores con vigencias y estado del procedimiento en `frontend/src/app/features/admin/indicators/` — **el valor se captura como `string` y pasa por `decimal.js`** (`indicator-form.ts`), nunca por `number`: `parseFloat` aceptaría `1e3` y `1.5000000000000002`, y en el segundo caso el redondeo ocurriría antes de que nadie pudiera avisar. La pantalla abre con el ESTADO del procedimiento y no solo con el formulario, porque olvidar la carga anual no rompe nada de inmediato: la calculadora que usa `@UVT` sigue funcionando con el valor del año anterior hasta que alguien mira una cifra mal. Los nombres ya registrados se ofrecen como atajos —una errata crea un indicador INVISIBLE, que existe en la tabla y que ninguna fórmula puede leer— y corregir una vigencia deshabilita el nombre, que es el vínculo con las fórmulas. 39 pruebas unitarias de la lógica pura, que son los MISMOS casos límite que rechaza el Simulador (cero permitido, seis decimales, ceros a la derecha que no cuentan, signo, rango de 14 dígitos, fin exclusivo) más una prueba e2e que recorre la cadena real hasta la base. **La regla de T171 encontró aquí tres falsos positivos** —`days_remaining` es un recuento de DÍAS y no una cifra monetaria, y `describeDays` recibía ese mismo entero—: en lugar de rodear la regla con `eslint-disable`, el archivo que declara la forma del CONTRATO queda excluido con la razón escrita en el propio tipo, y el recuento de días se convierte en la vista (la aritmética decimal no entra al componente). Y una prueba encontró una divergencia real: la forma canónica del cliente no admitía el signo, así que `-1` daba el mensaje de sintaxis en lugar de «un indicador no puede ser negativo»
- [X] T109 [US4] Advertencia visible antes de ejecutar una calculadora que dependa de indicadores sin vigencia (FR-062) — **vive en `features/simulators/` y no en `features/calculators/runner/`, que todavía no existe** (llega con el constructor visual, T097): hoy la única superficie de ejecución es el formulario, y ahí es donde la advertencia sirve de algo. La dependencia se DECLARA en `calculators.config.ts` (`indicators: ['UVT']` en el modo GMF) y la lista de lo que se quedó sin vigencia la publica el borde en `/indicators/current`, que es el catálogo de la plataforma al día. El aviso se DERIVA del modo elegido: al cambiar de calculadora —o de modo dentro de `colombia_especifica`— se recalcula solo, y por eso la prueba comprueba primero que en el modo de tasas NO aparece aunque el mismo indicador falte. No bloquea: en el camino nativo el valor lo escribe el usuario (`gmf` pide `valor_uvt`), así que lo que dice es que la plataforma no puede confirmarlo; y si la consulta falla, no hay aviso pero tampoco un error —el usuario vino a calcular—. 3 pruebas unitarias, incluida la de que un fallo de la consulta no impide calcular
- [X] T110 [P] [US4] Mostrar `indicators_used` y `calculator_version` en el historial de simulaciones — **la procedencia se añade al DTO del borde** (`GET /simulators/history`), que hasta ahora publicaba la simulación sin decir con qué se calculó: FR-050 y FR-058 existían en la base y no llegaban a ninguna pantalla. La versión se muestra como `versión 3` —una definición cambia con el tiempo y sin la versión una simulación de hace un año se explicaría con la fórmula de hoy— y **`0` significa «sin versión», no la versión cero**: las filas anteriores a la enmienda que la migración de T020 no pudo atribuir se calcularon con constantes cableadas, y «v0» sugeriría una definición que no existe. Los indicadores van en la columna de PARÁMETROS, porque son una entrada del cálculo que nadie tecleó ese día, y **sus valores se muestran canónicos y sin formatear**: el navegador no conoce el tipo de un indicador (`@UVT` es un monto y `@IPC` una tasa) y adivinarlo por el tamaño de la cifra convertiría un UVT mal cargado en 0,05 en una tasa con dos decimales, que es una cifra distinta de la almacenada. `indicators_used` sale como `{}` y nunca como `null` (`Object.entries(null)` falla, y una fila pobremente explicada no puede dejar la pantalla en blanco). 3 pruebas unitarias y 2 del borde
...
- [X] T118 [P] [US5] Listado de calculadoras propias con su estado de curaduría y el motivo del rechazo (FR-051…FR-054). **Cerrada al completarse T097**: la pantalla hace las dos mitades —proponer y leer el motivo— y el constructor, que era lo que faltaba para poder crear una, vive en `builder/` y se alcanza desde aquí Acción "proponer para publicación" y visualización del motivo de rechazo en `frontend/src/app/features/calculators/builder/`
...
- [X] T120 [P] [US6] Pruebas del validador de documento: nodo desconocido, marca desconocida, atributo no admitido, imagen sin `alt`, y `href` con `javascript:` — **44 casos en `services/learning/test/articles/body-doc.validator.spec.ts`, y 30 de los 44 comprueban RECHAZOS**, proporción deliberada: un validador que solo se prueba con documentos válidos pasa igual de verde si su cuerpo es `return doc`. Los seis casos de `href` son los que más justifican que el vocabulario sea cerrado: `jaVaScRiPt:` (mayúsculas mezcladas) y un `href` relativo caen los dos, y una lista negra de esquemas habría dejado pasar el primero. Se añaden casos que la tarea no pedía y que son los que un editor real provoca: un nodo hoja con hijos, un párrafo dentro de un párrafo, un atributo `html` colado, un anidamiento de 17 niveles y un documento de más de 5.000 nodos
- [X] T121 [P] [US6] Prueba de que se rechaza un archivo que no es imagen aunque declare un `mime_type` admitido, y uno que excede 2 MB (FR-066), en `services/learning/test/images/images.service.spec.ts` — **16 pruebas, y las imágenes son imágenes DE VERDAD**: se generan con `sharp` al vuelo y no con bytes escritos a mano, porque un PNG inventado con la cabecera correcta y el resto basura pasaría tanto si el servicio mira los bytes como si solo mirara la cabecera, y lo que hay que fijar es justo lo primero. Los rechazos: un ZIP que dice ser PNG, un PDF renombrado a JPEG, un GIF (formato real pero no admitido, con el mensaje diciendo cuál es y cuáles se admiten), una subida vacía, un archivo que pasa de 2 MB —con el mensaje en MB y en bytes—, un tipo declarado que no coincide con el real, y un `article_id` que no es UUID sin llegar a la base. Y se comprueba el ORDEN: con bytes que no son imagen Y pasan del tope, el mensaje habla del tamaño y no del formato, que es la única forma de ver desde fuera que la comprobación barata va primero. Dos casos dependían de particularidades de `pg-mem` —no reproduce el `rowCount` de un `ON CONFLICT DO NOTHING` que no inserta, y devuelve el `BYTEA` como texto—, así que se ajustaron a lo que el doble puede afirmar y **esas dos propiedades se verificaron contra PostgreSQL de verdad**
...
- [X] T123 [US6] Validador del vocabulario **cerrado** de research D-14 (nodos, marcas, atributos, esquemas de `href`), con validación positiva y negativa — en `services/learning/src/articles/body-doc.validator.ts`, sobre el vocabulario declarado aparte en `body-doc.ts`. **Negativo, no limpiador**: nada aquí intenta corregir un documento; si algo no encaja, la operación falla diciendo **qué** nodo, **qué** atributo y **en qué ruta** (`contenido[1].contenido[3]`), porque el documento lo escribe una máquina y lo depura una persona. **Un hueco propio, encontrado por mis propias pruebas**: un `encabezado` no estaba en la lista de contenedores, así que sus hijos no se recorrían y habría aceptado cualquier cosa dentro de un titular. Se corrigió, y con él dos decisiones de jerarquía que antes no existían: un elemento de lista admite párrafos, listas anidadas, imágenes y calculadoras —una viñeta con una imagen es contenido legítimo— pero **no** un encabezado, porque un `h3` dentro de una viñeta aparecería en el índice del documento como una sección y dejaría al artículo con una estructura que no es la que se ve. El `href` se resuelve con `URL` y se compara contra lista blanca: `javascript:`, `JaVaScRiPt:`, `data:`, `file:`, `vbscript:` y `blob:` se rechazan igual
- [X] T124 [US6] Persistir y devolver `body_doc` en creación, edición y lectura de versiones, en `services/learning/src/articles/` — **APLICADA EN CONTRATO, SERVIDOR Y BORDE, y verificada por capas.** El campo `body_doc` existe en `ArticleVersion`, `CreateDraftRequest`, `UpdateDraftRequest` y `Article` (para que el lector renderice bloques, T133), y viaja como **texto JSON** y no como mensaje anidado: con un mensaje, el vocabulario cerrado se validaría a medias —el esquema admite el nodo y el validador lo rechaza— y el mapeo del Gateway descartaría en silencio los atributos que no conoce, con lo que un cliente recibiría «guardado» habiendo perdido parte de su documento. Con texto, el borde transporta sin interpretar (Principio II) y Aprendizaje valida el árbol entero en un solo sitio, con la ruta del nodo culpable. **Stubs regenerados**: Go en los cinco servicios y TypeScript en Aprendizaje, Notificación y Frontend; **el Simulador en Rust NO se regeneró porque su proto no cambió, y se comprobó** (`services/simulator` sin diferencias). Se hizo replicando los comandos de `contracts/generate.sh` porque el guion comprueba las herramientas ANTES de empezar y sin `cargo` no llega ni al primer paso; `buf`, `protoc-gen-go`, `protoc-gen-go-grpc` y `protoc` se instalaron para esto. En el borde, `body_doc` sale como **documento y no como cadena** (`json.RawMessage`, para que el cliente no reciba un texto que tenga que volver a analizar) y **se omite** cuando no lo hay: la cadena vacía en el contrato significa «versión anterior al documento de bloques, cae a `body`», y confundirlo con «documento vacío» diría que esa versión tiene un cuerpo sin bloques, que es otra cosa. **Evidencia**: 231 pruebas unitarias de Aprendizaje, 10 nuevas de frontera (4 en Go para las decisiones de forma del borde, 6 en la serialización del lado del servidor) y las suites de extremo a extremo us1/us4 más la barrera de accesibilidad en verde contra las imágenes reconstruidas. **Lo que queda para T131**: que el documento lo ENVÍE el cliente en vez de derivarse del texto —hoy un `body_doc` vacío significa «derívalo tú» y así lo hace el servidor—; se deja para el editor porque es él quien define qué manda, y cablearlo antes sería inventarse su contrato de entrada
- [X] T125 [P] [US6] Repositorio de imágenes con direccionamiento por contenido (SHA-256) y deduplicación en `services/learning/src/images/images.repository.ts` — `INSERT … ON CONFLICT (id) DO NOTHING` y después `SELECT`, y no un «buscar primero e insertar después»: dos subidas simultáneas del mismo archivo calculan el mismo hash y las dos intentan insertar, así que la única forma de que la deduplicación sea correcta bajo concurrencia es intentar insertar y aceptar el conflicto. `rowCount` distingue «la subí» de «ya estaba». Consecuencia que conviene decir en voz alta: el mismo archivo subido a dos artículos es **una sola fila** y `article_id` guarda quién lo subió primero; FR-067 se cumple por el DOCUMENTO, porque la referencia vive en el nodo `imagen` de `body_doc`
- [X] T126 [US6] Servicio de imágenes: validación de bytes reales con `sharp`, lectura de dimensiones, cálculo del hash y tope de 2 MB, en `services/learning/src/images/images.service.ts` — **el tipo y el tamaño se comprueban contra los BYTES, no contra lo que declara el cliente**: un `Content-Type` y un nombre de archivo los elige quien sube, así que creerlos es no comprobar nada. El tope se mira ANTES de decodificar (la comprobación barata primero, para no gastar memoria en algo que se va a rechazar) y el identificador lo calcula el servidor: si lo mandara el cliente, podría subir una imagen con el identificador de otra y envenenar la deduplicación. Detalle que obligó a mirar el comportamiento real de la biblioteca: `sharp(bytes).metadata()` **no lanza** ante un archivo que no sea imagen —devuelve un objeto sin dimensiones y sin formato—, así que se comprueban explícitamente; un `try/catch` a secas habría dejado pasar cualquier cosa por la rama «no lanzó». Un tipo declarado que no coincide con el real **se rechaza en vez de corregirse en silencio**: el archivo es válido, pero quien sube un PNG llamándolo JPEG tiene un error en su cliente y merece saberlo
- [X] T127 [US6] `UploadArticleImage` y `GetArticleImage` en `services/learning/src/images/images.controller.ts` — con su mapeo aparte (`images/mapping.ts`) y el `byte_size` viajando como STRING aunque hoy quepa en un `number`: el contrato lo declara así para no tener que cambiarlo el día que un conteo grande no quepa en un entero seguro de JavaScript. `GetArticleImage` devuelve los BYTES junto a los metadatos porque quien los pide los va a servir o a dibujar, y separarlos obligaría a dos llamadas para una sola cosa. **Su prueba es la del servicio**, que corre contra la base real mediante sonda; el camino completo por el borde se verifica en T129/T130
- [X] T128 [US6] Extracción y validación de las referencias a imágenes del documento al guardar, en `services/learning/src/images/` y `src/publishing/` — **la extracción ya estaba; ahora está también el gancho, y entró cuando tocaba**. Se había dejado fuera a propósito porque el documento que llegaba se derivaba del texto (T124): validar referencias de contenido YA ESCRITO habría creado un fallo que nadie puede arreglar —el artículo está publicado y su imagen es anterior a la tabla—. Con el editor (T131) el documento empieza a llegar del cliente y el gancho pasa a tener sentido. `ImagesRepository.findExisting` + `ImagesService.findMissing` consultan qué identificadores existen, y `PublishingService.resolveBody` rechaza con `invalid_argument` **nombrando los que faltan**: sin el nombre, quien edita ve «hay una imagen rota» y tiene que adivinar cuál de las doce de su artículo es. El orden de las comprobaciones no es casual —forma del documento (sin tocar la base), documento no vacío y solo al final las referencias, que sí son una consulta—. **Verificado contra PostgreSQL real**: la consulta con marcadores es válida, `findMissing` distingue lo que existe, una referencia rota NO escribe nada (comparado el `body_doc` y el `body` antes y después) y una buena guarda el documento entero con su estructura y su negrita. Un detalle que costó: `ANY($1::text[])` **no es reproducible con el doble en memoria** —devuelve cero filas en silencio—, así que la consulta usa un marcador por identificador: una consulta que no se puede reproducir en una prueba es una consulta que acaba sin probar
- [X] T129 [US6] Ruta multiparte `POST /editorial/articles/{articleId}/images` con 413 y 415, en `services/api-gateway/internal/handler/media.go` — **7 pruebas del borde, con los dos estados que la tarea pide.** 413 con `http.MaxBytesReader`: el cuerpo se corta DURANTE la lectura, así que una subida de 200 MB no se queda en memoria para acabar rechazada —sin esto, el límite del servicio llegaría cuando el borde ya hubiera cargado el archivo entero—. 415 por el tipo declarado, que es el estado que HTTP define para «no lo atiendo»; devolver 400 para los dos dejaría al cliente sin saber cuál de las dos cosas arreglar. Y las dos comprobaciones NO son la misma que la de Aprendizaje repetida: aquí se sabe antes de mover bytes, y **si el archivo MIENTE sobre su tipo es allí donde se rechaza con 400 y su motivo**, porque el tipo declarado lo elige quien sube y solo abrir los bytes dice la verdad. Se comprueba además que la ruta exija rol de editor, que sin el campo `file` sea 400 con el nombre del campo que falta, y que el `uploaded_by` salga de los **claims verificados** y no de la petición —que es lo que impide subir una imagen «como si fuera» otro—. Dos defectos propios de la PRUEBA, encontrados al ejecutarla: `CreateFormFile` fija `application/octet-stream` y no deja variar el tipo declarado —así que la prueba del 415 medía siempre el mismo valor— y hubo que usar `CreatePart` con la cabecera explícita; y una comprobación de `ByteSize` sobre el mensaje de petición, donde ese campo no existe porque es de la respuesta
- [X] T130 [US6] Ruta `GET /media/images/{imageId}` con `ETag` y `Cache-Control: public, max-age=31536000, immutable`, en `services/api-gateway/internal/handler/media.go` — **verificada por HTTP REAL, sin token**, contra el borde reconstruido: `200`, `Content-Type: image/png`, `ETag` igual al identificador, la cabecera de caché exacta, 95 bytes recibidos y `file` confirmando *PNG image data, 12 x 7*; la segunda petición con `If-None-Match` devuelve **304 con 0 bytes**, y un identificador inexistente da 404. **La ruta es PÚBLICA y es una decisión escrita, no un descuido**: el `<img src>` del lector no puede mandar cabecera `Authorization`, así que una ruta autenticada haría que ninguna imagen se viera nunca; el identificador es el SHA-256 del contenido —256 bits que no se adivinan— y solo lo conoce quien tiene el documento que lo referencia, de modo que **saber el hash ES el permiso**. La alternativa canónica son URLs firmadas, que exigen infraestructura que esta enmienda no añade (D-13). Se añadió además el `304`, que no pedía la tarea: un `ETag` sin `If-None-Match` es un dato que se anuncia y no se usa. Y se inscribió la ruta en `TestPublicRoutesNeedNoToken`, la lista que documenta qué superficie es alcanzable sin credenciales
- [X] T131 [US6] Editor TipTap con barra de herramientas y esquema restringido al vocabulario de D-14, en `frontend/src/app/features/editorial/editor/`, sustituyendo el `<textarea rows="8">` — **hecho y verificado en el navegador**. El esquema es la primera barrera: de las extensiones de TipTap se apagan cita, código, bloque de código, línea horizontal, tachado, subrayado y **salto de línea** —el vocabulario no tiene nodo para él: si existiera, Mayús+Intro produciría un nodo que al guardar desaparecería sin avisar—, los encabezados se limitan a 2–4 y los enlaces a `http`/`https`/`mailto`. No es «desactivar botones que dan error»: no existen los nodos. Dos conversiones y solo dos (`toBodyDoc` al guardar, `toEditorDoc` al abrir`) traducen entre ProseMirror y el vocabulario, y **guardar sin tocar nada devuelve exactamente el mismo documento** —la propiedad que impide que abrir un artículo lo modifique—, fijada con pruebas de ida y vuelta sobre documentos anidados. Un `ControlValueAccessor`: el formulario recibe un documento, no texto, y la regla «el cuerpo no puede estar vacío» se comprueba antes de enviar. **La petición lleva SOLO el documento**: el texto de `body` lo deriva el servidor, porque mandar los dos dejaría dos versiones del mismo cuerpo que pueden contradecirse. 21 pruebas de conversión + 16 del componente
- [X] T132 [US6] Nodo `imagen` con `alt` **obligatorio** y pie de foto opcional, e inserción con subida, en `frontend/src/app/features/editorial/editor/` — **verificado de extremo a extremo en el navegador** por una prueba nueva (`e2e/editor-imagen.spec.ts`): el editor sube un PNG real por `multipart`, el borde lo reenvía, Aprendizaje valida los BYTES y lo guarda direccionado por su hash, el documento se guarda con la referencia, un coordinador distinto publica y **el lector pinta la imagen con su `alt` y su pie** (`naturalWidth` 640, no solo «visible»: un `<img>` que no carga sigue ocupando su sitio). El nodo es **atómico** y el `alt` es un ATRIBUTO que se pide al insertar: si fuera texto dentro del documento, seleccionarlo y borrarlo dejaría una imagen sin descripción y el rechazo del servidor hablaría de un nodo que quien escribe no ve. La imagen se sube al elegirla y se describe DESPUÉS, para que la vista previa sea la imagen que servirá el servidor y no una del disco; en un artículo que todavía no existe el campo está desactivado y dice por qué (una imagen pertenece a un artículo). Tres defectos propios encontrados al probarlo: el `<input type="file">` oculto **no tenía nombre accesible** —lo cazó la barrera de accesibilidad, y era cierto: había un botón con `aria-label` pulsando el campo por detrás, así que el control alcanzable con el teclado no se anunciaba— y las cuatro opciones de una pregunta se llamaban solo por su `placeholder`, que desaparece al escribir
- [X] T133 [US6] Render del documento **por componente Angular, sin `innerHTML` ni `bypassSecurityTrust*`**, en `frontend/src/app/features/learning/article/blocks/` (FR-068, nota N-08) — **HECHA y verificada EN EL NAVEGADOR.** Un `<ng-template>` que se invoca a sí mismo con `ngTemplateOutlet` (Angular no deja que un componente independiente se importe a sí mismo, y un componente por nivel obligaría a un anidamiento fijo): el contexto lleva si se está dibujando contenido en línea o en bloque, así que hay **una sola lista de tipos** y no dos que puedan divergir. Cada nodo emite su elemento —`<p>`, `<h2>/<h3>/<h4>`, `<ul>/<ol>`, `<li>`, `<figure>`— y la única superficie que existe es la escrita: no hay analizador de HTML en el camino al que colarle nada. **El respaldo en texto NO se retira**: hay versiones publicadas antes del documento de bloques —de verdad, no en teoría— y son caminos excluyentes, porque renderizar los dos mostraría el cuerpo dos veces. **La seguridad se comprueba en los dos extremos**: `isSafeHref` repite en el cliente la lista blanca de esquemas del servidor, con `URL` y no con lista negra, porque una garantía que solo vive al otro lado del cable no protege a esta pantalla si el otro lado cambia o si el documento se guardó antes de que existiera el validador; un `javascript:` no se dibuja como enlace y su texto tampoco se pierde. La imagen apunta a `GET /media/images/{id}` (T129/T130, pendiente) y mientras no exista muestra su **texto alternativo** como pie visible: es la degradación correcta cuando una imagen falta y un hueco en blanco sería peor. La calculadora se dibuja como referencia con enlace al simulador; T153 la vuelve ejecutable en el lector. **Evidencia**: 23 pruebas unitarias nuevas (182 en total) —entre ellas que un `<script>` en el texto aparece como TEXTO y no crea ningún elemento, y que `JaVaScRiPt:` no produce enlace—, la aserción de la suite visual de que el navegador está renderizando el documento (`.fc-blocks`), y **45/45 de extremo a extremo**. Se añadió además la barrera `frontend/scripts/security-barrier.mjs` (T157) porque no existe regla de plantilla que prohíba `[innerHTML]`
- [X] T134 [P] [US6] Prueba e2e de que un documento con nodo o enlace no admitido se rechaza al guardar y nunca llega al lector, en `frontend/e2e/body-doc-rechazado.spec.ts` — **el editor no puede producir un documento así** (su esquema no tiene esos nodos, y esa es la primera barrera), así que la prueba hace lo único que puede hacer un cliente hostil: intercepta el `PATCH` de la propia aplicación y sustituye el documento en vuelo. La petición sigue siendo la de la aplicación —con su sesión, su ruta y su forma— contra el borde real, que es lo que hay que comprobar; un `fetch` con un token inventado no probaría nada del camino que usa la aplicación. Tres familias: un nodo que no existe, un enlace `javascript:` y un atributo de más en un nodo válido (el que más fácil se cuela: `{tipo:'parrafo', onload:…}` es un párrafo legítimo con una carga encima). Se comprueban las dos mitades de FR-068, y la segunda es la que importa: el servidor rechaza con 400 **y el borrador queda exactamente como estaba** —reabriendo la versión por su dirección, no recargando, que en modo crear volvería al formulario vacío— y el contenido inyectado no aparece por ninguna parte
...
- [X] T157 [P] Regla de lint que prohíba `innerHTML` y `bypassSecurityTrust*` en `frontend/src/app/features/learning/` — la verificación por `grep` del quickstart §6 promovida a barrera automática — **hecha en DOS mitades, porque hay que cubrir dos caminos.** Para TypeScript, reglas de ESLint: `no-restricted-syntax` sobre `.innerHTML`, `.outerHTML`, `.insertAdjacentHTML` y `bypassSecurityTrust*`, más `no-restricted-imports` sobre `DomSanitizer` (no es peligroso solo, pero solo se inyecta para lo anterior). Para las **plantillas** no existe regla que prohíba `[innerHTML]` —se comprobó listando las del plugin de `angular-eslint`, y no está—, así que la segunda mitad es **`frontend/scripts/security-barrier.mjs`**, que corre dentro de `npm run lint`. Se aplica a todo `src/app/**` y no solo a `features/learning/`: acotarla a un directorio la dejaría caducada en cuanto el editor (T131) cree su propia carpeta. **Dos defectos propios, los dos encontrados ejecutándola y no leyéndola**: (1) la primera versión denunciaba **los comentarios que explican por qué no se usa** `bypassSecurityTrust*`, así que ahora distingue código de comentario con un recorrido carácter a carácter —y no con una expresión por línea, porque las trampas están dentro de las cadenas: `'https://x'` no abre un comentario—; (2) `\b\[innerHTML\]` **no coincidía nunca**, porque entre un espacio y un corchete no hay límite de palabra, de modo que el caso más importante era justo el que no funcionaba. Por eso la barrera ahora **se autocomprueba con 13 casos** en cada ejecución: una barrera que puede dejar de funcionar en silencio y seguir diciendo «0 usos prohibidos» es peor que no tenerla. Comprobada con sondas reales en ambos sentidos (detecta `[innerHTML]` y `.innerHTML`; ignora los comentarios)
- [X] T158 [P] Extender la regla de análisis estático anti-punto-flotante a `services/simulator/src/domain/formula/` (Constitución §Calidad y Pruebas) — **CERRADA SIN CAMBIOS, y con la prueba de que hacía falta** (2026-09-17): la tarea pedía «extender la regla», y la regla **ya cubría ese directorio** porque no está acotada a ningún módulo: `#![deny(clippy::disallowed_types)]` se declara en la RAÍZ de `lib.rs` y `main.rs`, y una regla de crate alcanza a todos sus módulos. Se comprobó de la única forma que vale, metiendo una violación de verdad: un `pub fn malo() -> f64` en `src/domain/formula/` hizo fallar `cargo clippy --all-targets -- -D warnings` con «use of a disallowed type f64» señalando el archivo, en la lib y en la lib test. Un `#[allow]` habría dejado la duda; la violación la responde.
- [ ] T159 [P] Documentar la gramática del lenguaje de fórmulas, con ejemplos y la diferencia entre `pot` y `potd`, en `docs/`
- [X] T160 [P] Ayuda contextual en el constructor que explique cuándo usar `pot` y cuándo `potd`, en `frontend/src/app/features/calculators/builder/` — **HECHA (2026-09-17)**: `builder/formula-help.ts` con las dos funciones y **una barrera que impide que el texto divergiera del lenguaje**: `frontend/scripts/formula-help.mjs` compara la ayuda visible con `Function::help()` del Simulador —que es donde vive el texto autorizado, decidido en T083— y falla si alguna deja de afirmar que `pot` quiere exponente ENTERO y es exacta, o que `potd` es decimal y aproximada. Corre en `npm run lint`, junto a las otras dos barreras. **La barrera se probó rompiéndola en las dos direcciones** (borrando «aproximada» de la ayuda visible, y cambiando «ENTERO» en el Rust): las dos veces falló y con el mensaje que dice qué corregir. Dos veces se descubrió que la propia barrera era el defecto —buscaba la ayuda en el archivo entero y encontraba el `name()` de la función, y su expresión regular se comía el brazo siguiente por ser voraz— y las dos se arregló la comprobación, no lo que comprobaba. La ayuda se pinta DENTRO del módulo de resultados, que es el momento exacto en que hace falta.
...
- [X] T170 [P] Regla de lint que prohíba el atributo `style="..."` en las plantillas de `frontend/src/app/features/{admin,calculators}/` y en los bloques de artículo — **ya estaba cubierta, y por más de lo que pedía**: la regla `@angular-eslint/template/no-inline-styles` está declarada sobre `*.html` **de todo el proyecto**, no por directorio, así que las pantallas nuevas nacen sin deuda de estilo sin necesidad de añadir nada. Se verificó que la regla está activa y que el componente nuevo pasa limpio, en vez de dar por hecho que «estar en el config» es lo mismo que «aplicarse». Los 8 errores que quedan en el lint son, todos, las plantillas del editor, que pertenecen a 002 y esperan a T131. Nota para `admin/indicators` y `calculators`: cuando se escriban (T108, T097) heredan la regla sin hacer nada
- [X] T171 [P] Regla de análisis estático que prohíba `number`, `parseFloat` y `Number()` sobre montos, tasas y valores de indicador en `frontend/src/app/features/{calculators,admin/indicators,learning/article/blocks}/` — **aplicada, con un ajuste que la propia regla obligó a hacer.** Para `calculators` y `admin/indicators` se activa tal cual está la de los simuladores (veto al TIPO `number`, a `parseFloat`/`Number` y a `Math.round`). Para `learning/article/blocks` el veto al tipo `number` **se retiró**, y no por comodidad: la primera pasada acusó a `body-doc.ts` por declarar `nivel` (2–4) y `version` (un entero), que **no son dinero**, y un directorio de renderizado de bloques necesita enteros estructurales. Se queda con el veto a las **conversiones** (`parseFloat`, `Number`, `parseInt`, `Math.round`), que es donde está el riesgo de verdad: el dinero que este directorio dibuja llega como **cadena decimal canónica** desde la API y no se convierte aquí. Aplicar la regla a la letra habría obligado a poner `eslint-disable` en código correcto, que es como empieza el final de una regla
