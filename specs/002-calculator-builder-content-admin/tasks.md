...
- [ ] T124 [US6] Persistir y devolver `body_doc` en creación, edición y lectura de versiones, en `services/learning/src/articles/` — **HECHA LA MITAD DE PERSISTIR, y la de devolver está bloqueada por el contrato.** Lo que está: `plainTextToBodyDoc` (`src/articles/plain-text.ts`) aplica la MISMA regla que la migración de T016 —un párrafo por bloque separado por línea en blanco—, y el camino de escritura del repositorio la usa en `createArticle`, `createNewVersion` y `updateDraftBody`, de modo que **toda versión nueva nace con su documento**; `body` y `body_doc` se guardan juntos y `VersionRow` devuelve los dos. **Verificado contra la base REAL, no contra el doble de pruebas**: tras reconstruir la imagen, la suite editorial creó un artículo por la API y su versión quedó con el documento y el texto exactos. Además se añadió la migración `20260902114500_backfill_body_doc_nulls`, porque entre T016 y T124 quedaron 14 versiones —las que crearon las pruebas de extremo a extremo en ese intervalo— con `body` y sin documento: el invariante se restauró a **94 de 94**, y se hizo como paso propio para que T135 pueda ser lo que dice ser en lugar de llevar un `COALESCE` defensivo que dejaría documentos vacíos con aspecto de contenido. **Lo que falta**: devolverlo por el contrato. `ArticleVersion`, `CreateDraftRequest` y `UpdateDraftRequest` necesitan el campo `body_doc`, y `Article` también para que el lector renderice bloques (T133); eso exige regenerar los stubs con `contracts/generate.sh`, y **la cadena de generación no está instalada en esta máquina** (`buf`, `protoc-gen-go`, `protoc-gen-go-grpc` y `protoc` no están en el PATH; `go` sí). Se midió en vez de suponerlo. No se escribió a mano el stub generado: es un archivo derivado y editarlo a mano es exactamente la clase de atajo que este libro ya tiene anotada dos veces
