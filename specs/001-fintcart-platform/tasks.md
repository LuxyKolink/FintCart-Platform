...
- [X] T154 [P] [US4] Prueba de contrato gRPC de `LearningService` editorial (`CreateDraft`, `UpdateDraft`, `SubmitForReview`, `ApproveAndPublish`, `Archive`) en `services/learning/test/editorial.contract.spec.ts`
- [X] T155 [P] [US4] Prueba de la invariante de separación de responsabilidades — `approved_by ≠ created_by` — incluyendo el intento de un editor de publicar su propio artículo, en `services/learning/test/publishing.spec.ts` (FR-008)
- [X] T156 [P] [US4] Prueba e2e Playwright del recorrido editorial con dos actores distintos en `frontend/e2e/us4-editorial.spec.ts`

### Implementación — Flujo editorial (Aprendizaje)

- [X] T157 [US4] Implementar `LearningService.CreateDraft` y `UpdateDraft` creando filas de `article_versions` en estado `borrador` visibles solo a su editor, en `services/learning/src/publishing/publishing.service.ts` (FR-007)
- [X] T158 [US4] Implementar `LearningService.SubmitForReview` con la transición `borrador → en_revision` en `services/learning/src/publishing/publishing.service.ts` (FR-008)
- [X] T159 [US4] Implementar `LearningService.ApproveAndPublish` con la transición `en_revision → publicado`, rechazando la operación cuando `approved_by == created_by`, y actualizando `articles.current_version_id` dentro de una transacción, en `services/learning/src/publishing/publishing.service.ts` (FR-008)
- [X] T160 [US4] Implementar `LearningService.Archive` con la transición `publicado → archivado` en `services/learning/src/publishing/publishing.service.ts`
- [X] T161 [US4] Implementar el versionado incremental (`version_no`) que preserva la trazabilidad histórica al generar una nueva versión de un artículo publicado, en `services/learning/src/publishing/versioning.service.ts` (FR-013)
- [X] T162 [US4] Implementar la gestión de cuestionarios asociados al artículo (≥ 1 por artículo) en el flujo editorial, en `services/learning/src/quizzes/quizzes.service.ts` (FR-009)
- [X] T163 [US4] Implementar la publicación del evento `learning.article_published` hacia Auditoría en `services/learning/src/events/publisher.ts` (Principio V, FR-008)
- [X] T164 [P] [US4] ~~Implementar el consumidor de `learning.article_published` y de `user.activity`~~ — no aplicable bajo la arquitectura vigente; ver nota

### Implementación — Autorización, borde REST y frontend

- [X] T165 [US4] Implementar la aplicación de los roles `editor` y `coordinador_editorial` en los middlewares de autorización de las rutas editoriales, en `services/api-gateway/internal/handler/middleware.go` (FR-006)
- [X] T166 [US4] Implementar los handlers REST `POST /editorial/articles`, `POST /editorial/versions/{versionId}/submit` y `POST /editorial/versions/{versionId}/publish` en `services/api-gateway/internal/handler/editorial.go`, más `POST /editorial/articles/{articleId}/versions`, `PATCH /editorial/versions/{versionId}`, `POST /editorial/versions/{versionId}/archive`, `GET /editorial/versions`, `POST /editorial/quizzes` y `PUT /editorial/quizzes/{quizId}`
- [X] T167 [P] [US4] Implementar el editor de artículos y cuestionarios en `frontend/src/app/features/editorial/editor/` (FR-007, FR-009)
- [X] T168 [P] [US4] Implementar la bandeja de revisión del coordinador editorial en `frontend/src/app/features/editorial/review/` (FR-008)
- [X] T169 [US4] Implementar la vista de historial de versiones de un artículo en `frontend/src/app/features/editorial/versions/` (FR-013)
- [X] T170 [US4] Implementar el guard de rol que oculta las rutas editoriales a usuarios finales — `frontend/src/app/core/auth/role.guard.ts`, no un `editorial.guard.ts` nuevo; ver nota (FR-006)
- [X] T171 [P] [US4] Escribir pruebas de persistencia editorial contra driver SQL simulado en `services/learning/test/publishing.repository.spec.ts`

**Notas de T154–T171 (US4)**:

- **Contrato ampliado antes de implementar** (`learning.proto`, regenerado en los cinco servicios Go + los tres destinos TS, verificado con `buf lint` + `go build`/`tsc --noEmit` en los ocho): `CreateDraftRequest` gana `article_id` (vacío ⇒ artículo nuevo; relleno ⇒ nueva versión de uno existente, FR-013 — no había NINGÚN campo para pedir esto); `ArticleVersion` gana `created_at`/`published_at`/`body` (el historial y "seguir editando" un borrador exigen leer de vuelta lo que se guardó, y no había otro RPC que lo devolviera); nuevo RPC `ListVersions` (historial de un artículo, bandeja de revisión y borradores propios son la MISMA consulta con distintos filtros — mismo patrón que `ListAttempts.quiz_id` vacío); nuevo RPC `UpsertQuiz` (no existía NINGÚN RPC para crear un cuestionario — sin él, el Independent Test de US4 "editor crea artículo + cuestionario" era literalmente irrealizable). `UpsertQuiz` reemplaza el cuestionario COMPLETO en una llamada (`DELETE` + `INSERT` de sus preguntas en una transacción), no un CRUD por pregunta — evita un cuestionario a medio editar visible entre dos llamadas.
- **T163 — cambio de contrato respecto al enunciado de la tarea**: `learning.article_published` se publica solo hacia `audit.q`, no hacia Notificación. La topología (`orchestrator/internal/events/topology.go::BindingsNotification`, fijada en Foundational/T062) y `events-catalog.md` YA documentaban esta decisión — la asignación original a Notificación era anterior a la aclaración N-03 (la bandeja in-app la sirve Usuarios, no Notificación, que es consumidor puro sin gRPC) — así que T163 sigue la arquitectura ya vigente en vez de abrir un binding que el propio Foundational cerró a propósito.
- **T164 — NO APLICABLE, no un archivo vacío ni un stub**: pedía un consumidor en Notificación para `learning.article_published`/`user.activity` que generara notificaciones IN-APP. Es arquitectónicamente imposible bajo las restricciones vigentes: (1) ningún evento de los dos está enlazado a `notification.q` (ver la nota de T163); (2) aunque lo estuviera, Notificación es consumidor puro SIN gRPC (`plan.md` N-01) y no puede llamar a `Users.AppendInAppNotification`, el único punto de escritura de la bandeja. No se creó `activity.consumer.ts` porque habría sido una ruta de código muerto —nunca alcanzada por ningún binding— que simularía cobertura sin tenerla.
- **T165/T166 (las tres rutas originales) ya estaban implementadas** desde T055–T059 (Foundational): `CreateDraft`/`SubmitForReview`/`ApproveAndPublish` y su `RequireRole` ya existían con la separación de grupos que exige FR-008. Lo que sí se construyó aquí son las SEIS rutas que faltaban para que el flujo editorial completo (editar, archivar, listar/historial, cuestionario) fuera alcanzable desde la SPA — sin ellas T167–T169 no tenían con qué hablar. Recuento de `routes.go` actualizado de 22/23 a 28/29 (rutas/operaciones).
- **T170 — mismo patrón de desviación que T129/T138 en US3**: `frontend/src/app/core/auth/role.guard.ts` ya existía (`roleGuard(...allowed)`, genérico, comprueba autenticación y rol) desde una fase anterior. Crear `editorial.guard.ts` habría sido una segunda implementación del mismo mecanismo. Las cinco rutas `/editorial/*` usan `roleGuard('editor', 'coordinador_editorial')` o `roleGuard('coordinador_editorial')` según corresponda.
- **Diseño del editor (T167)**: un cuestionario cuelga del ARTÍCULO (`quizzes.article_id`), no de la versión — así que el gestor de cuestionario es idéntico en modo crear y en modo editar, y aparece en cuanto se conoce `article_id`. `UpsertQuiz` con `quiz_id` vacío SIEMPRE crea uno nuevo (el esquema no impide varios cuestionarios por artículo); el componente recuerda en memoria el `quiz_id` de la última creación/actualización de la sesión para que un segundo guardado sea `PUT` y no un segundo `POST`. Reabrir un borrador de una sesión anterior para seguir editando su cuestionario existente no está cubierto: no hay ningún RPC que liste los cuestionarios de un artículo (solo `GetQuiz` por id, que exige conocerlo). Queda anotado como hueco conocido, no resuelto — el Independent Test no lo requiere.
- **Bandeja de revisión (T168) no muestra el TÍTULO del artículo**: `ArticleVersion` no lo lleva (vive en `articles`, no en `article_versions`, y `ListVersions` no hace `JOIN`). Se muestra un fragmento del cuerpo como identificador provisional. Añadir el título exigiría otro cambio de contrato que ninguna tarea pedía explícitamente; queda anotado.
- **T171**: pruebas de `PublishingRepository` contra `pg-mem`, igual convención que `repositories.spec.ts` (T082). Encontró y corrigió un defecto real del esquema de pruebas: `articles`/`article_versions`/`quizzes`/`questions` dependían del `DEFAULT gen_random_uuid()` de PostgreSQL real para su `id`, que el esquema de `pg-mem` de `memdb.ts` no reproduce en esas tablas — los `INSERT` de `publishing.repository.ts` y `quizzes.repository.ts` pasan ahora `gen_random_uuid()` EXPLÍCITO en el `VALUES`, válido en los dos motores y ya no depende de esa particularidad del doble de pruebas.
- **Verificado en vivo contra `dev/up`** (`us4-editorial.spec.ts`, T156): registro y verificación de DOS cuentas distintas → `dev/seed role <correo> <rol>` concede `editor` y `coordinador_editorial` (no existe, ni debe existir, autopostulación — spec.md línea 235) → el editor crea un borrador con su cuestionario y lo envía a revisión → el coordinador lo aprueba y publica desde la bandeja de revisión → el artículo aparece en `/catalogo`. Passing en la primera ejecución real. Se re-corrieron `us1`/`us2`/`us3` en el mismo lote sin regresiones. `go build`/`go vet`/`go test`/`golangci-lint` limpios en los cinco servicios Go; `tsc --noEmit`/`eslint --max-warnings 0`/`jest` limpios en Aprendizaje (117 pruebas); `tsc --noEmit`/`eslint --max-warnings 0`/`ng build --configuration production` limpios en el Frontend.

**Checkpoint**: Las cuatro historias de usuario funcionan de forma independiente — verificado en el backend (seis servicios) y en ejecución de punta a punta contra `dev/up`, incluidas las cuatro specs Playwright juntas sin regresiones.
...
- [X] T172 [P] Escribir las pruebas de esquema de evento (productor) y de consumo idempotente (Notificación/Auditoría) para los **11** eventos de `contracts/events/events-catalog.md`, en `services/audit/internal/handler/consumer_test.go` y `services/notification/test/consumers.spec.ts` (D-13)
- [X] T173 [P] Verificar la inmutabilidad de `audit_log` con una prueba que confirme que `UPDATE` y `DELETE` fallan, en `services/audit/internal/storer/immutability_test.go` (FR-025)
- [X] T174 [P] Implementar y verificar el particionado anual de `audit_log` con retención ≥ 5 años en `services/audit/migrations/<ts>_partition_audit.{up,down}.sql` (FR-031)

**Notas de T172–T174**:

- **T172**: `consumer_test.go` gana `TestAllElevenCatalogEventsAreAcceptedAndStored` (una tabla con los 11 `event_type` de `events-catalog.md` y sus payloads mínimos, verificando que cada uno se acepta, se guarda íntegro en `context` y que `score` — cuando aparece — cruza como `string`, nunca como número JSON, Principio VIII) y `TestDuplicateDeliveryIsIdempotent` (la misma entrega dos veces: una sola fila, las dos confirmaciones). El doble `fakeStore` tuvo que aprender a deduplicar por `id+occurred_at` — igual que el `ON CONFLICT DO NOTHING` real de `storer_postgres.go` — porque sin eso la prueba de idempotencia pasaba contra un doble que no reproducía el comportamiento real. `consumers.spec.ts` (nuevo) hace el mismo par de verificaciones en Notificación: de los 11 eventos, los 3 con plantilla de email producen una notificación encolable con la plantilla correcta; los 8 restantes son válidos pero no producen correo (N-03, la bandeja in-app la sirve Usuarios) — ninguno de los dos desenlaces es un error; y una entrega duplicada encola una sola vez.
- **T173/T174 — un solo archivo, no dos, con desviación deliberada de la ruta literal de T173**: la tarea pedía `services/audit/internal/storer/immutability_test.go`, pero verificar un `REVOKE`/trigger real y el enrutado de una fila a su partición exige SQL ejecutándose contra un PostgreSQL de verdad — `go-sqlmock` (usado en el resto de la suite de persistencia) no ejecuta SQL, solo empareja las cadenas que el código emite, así que no puede comprobar una restricción impuesta por la base. Se creó `postgres_integration_test.go` (build tag `integration`, aislado de `go test ./...`) que levanta un PostgreSQL 16 efímero vía `testcontainers-go`, aplica las migraciones reales del servicio (leídas del disco, no reescritas a mano) y prueba T173 (`UPDATE`/`DELETE` sobre `audit_log` Y directamente sobre una partición child fallan) y T174 en el mismo archivo, porque ambas exigen la misma infraestructura de contenedor real y separarlas habría duplicado el setup sin aislar nada. `go.mod` ganó `testcontainers-go` como dependencia de test.
- **T174 — hallazgo real**: el esquema inicial (`20260727120000`, Foundational) YA particionaba `audit_log` por año con particiones 2026/2027 + una `DEFAULT` de resguardo y el trigger de inmutabilidad. Cubría 2 años, no los ≥5 exigidos por FR-031. La migración nueva (`20260815090000_partition_audit_rotation`) extiende la cobertura a 2030 y añade `audit_log_ensure_partition(year)`, una función idempotente pensada para que una tarea operativa anual (fuera de alcance: no hay orquestador de cron en el plan) cree la partición del año siguiente sin reescribir SQL a mano — repitiendo el `REVOKE` por partición, que no se hereda solo de la tabla particionada aunque el trigger sí cubra todas las hijas automáticamente.
- **Verificado con Docker real** (`go test -tags=integration ./internal/storer/...`): 6 pruebas, PASS, cada una contra su propio contenedor `postgres:16-alpine` efímero (testcontainers). `go build`/`go vet`/`gofmt -l`/`golangci-lint run --build-tags=integration` limpios en Auditoría; `npm test`/`npm run lint` limpios en Notificación (46 pruebas). CI (`ci.yml`, job `go`) gana un paso condicionado a `matrix.service == 'audit'` que corre la suite de integración contra el Docker preinstalado de `ubuntu-latest`, y el paso de lint pasa `--build-tags=integration` en los cinco servicios (sin efecto en los otros cuatro, que no tienen archivos con ese tag) para que el nuevo archivo no quede fuera del lint en CI.
- [X] T175 Ejecutar pruebas de carga con 1.000 usuarios concurrentes verificando respuesta percibida < 2 s y lecturas < 1 s, con el guion en `deploy/loadtest/k6-scenarios.js` (SC-003, SC-005)

**Notas de T175**:

- **El guion mide contra `dev/up` porque es lo que hay, no porque sea lo que SC-003/SC-005 describen**: los dos criterios hablan de la plataforma DESPLEGADA (`deploy/k8s`, ≥ 2 réplicas en ruta crítica, HPA). `dev/up` es una sola réplica de cada servicio en un portátil; una corrida ahí mide el portátil, no el sistema. El guion (`deploy/loadtest/k6-scenarios.js`) es el mismo contra los dos entornos — cambia `LOADTEST_BASE_URL` — y esta nota documenta qué cuenta como verificación real del criterio y qué es solo evidencia de que el guion funciona.
- **`setup()` crea un fondo de cuentas reales, no 1.000**: registrar una cuenta dispara la Saga de registro completa (Usuarios + Auth + correo) y verificarla exige sondear Mailhog. Repetirlo 1.000 veces al arrancar la prueba mediría esa saga, no las lecturas que SC-003/SC-005 exigen. `setup()` registra, verifica y autentica un fondo configurable (`LOADTEST_USER_POOL`) una sola vez, con el mismo flujo que `dev/demo`, y los VU lo reutilizan — como en la vida real, donde muchas pestañas concurrentes pertenecen a una fracción mucho menor de cuentas.
- **Cada VU manda su propio `X-Forwarded-For`**: el Gateway limita por IP (`internal/ratelimit`, clave `ip:` + la cabecera — confía en ella porque corre siempre detrás de un proxy, Principio X), y en producción 1.000 usuarios concurrentes son 1.000 IPs distintas. Sin una IP sintética por VU, la prueba entera habría compartido la IP del contenedor de k6 y habría medido el limitador de tasa por IP, no la latencia.
- **Hallazgo de metodología: `K6_VUS` colisiona con una opción nativa de k6**. La primera corrida a 1.000 VUs terminó en 7 segundos en vez de sostener la carga el tiempo configurado, sin ningún error. Causa: k6 trata CUALQUIER variable de entorno `K6_<OPCION>` como su propia opción de CLI (`K6_VUS` equivale a `--vus`). Al fijarla, k6 abandonó en silencio las tres etapas de `options.scenarios` y ejecutó su modo antiguo de una iteración por VU. Los nombres de configuración propios del guion se movieron al prefijo `LOADTEST_`, reservando `K6_` solo para lo que k6 ya define (`K6_SETUP_TIMEOUT`, usado a propósito con su nombre real). El guion documenta esto en cabecera para que nadie repita el error.
- **Hallazgo real, corregido: `QuizzesRepository.storeAttempt` no resistía la concurrencia que la propia carga generó**. Con 100 VUs contra un fondo de 15 cuentas (varios VU por cuenta, como el diseño del guion asume), un 8% de los envíos de cuestionario devolvía 500. La causa NO es la de T176 (esa ya estaba resuelta): dos transacciones concurrentes de la MISMA cuenta calculan el mismo `MAX(attempt_no)+1` y la segunda choca contra `UNIQUE (user_id, quiz_id, attempt_no)` — el propio comentario de `NEXT_ATTEMPT_NO_SQL` ya preveía esto y por eso existía un reintento (`ATTEMPT_RETRIES = 3`), pero 3 intentos sin espera entre ellos no alcanzaban bajo la concurrencia real de la prueba: la transacción que pierde la carrera reintenta antes de que la ganadora haya confirmado, y vuelve a chocar. Corregido subiendo a 10 reintentos con espera exponencial corta y aleatoria (`backoff`, en `services/learning/src/quizzes/quizzes.repository.ts`) — reduce la probabilidad de que dos transacciones seguidas compitan por la misma ventana sin convertir el caso normal (dos pestañas) en una espera perceptible. Verificado: 119/119 pruebas de Aprendizaje, lint y `tsc` limpios; la misma carga de 100 VUs que producía 8% de fallos pasó a 0% tras reconstruir el servicio.
- **Resultados, carga sostenida real (1.000 VUs, fondo de 100 cuentas, `ramping-vus` con rampa 20 s / sostenida 30 s / bajada 10 s, contra `dev/up`)**: `fintcart_read_duration` p95 = 434 ms (< 1 s, SC-003 ✓); `fintcart_action_duration` p95 = 1,66 s (< 2 s, SC-005 ✓); 6,86% de las peticiones devolvió 429. Investigado hasta la causa: NO es un defecto — es `RateLimitByUser` (un SEGUNDO limitador, por identidad autenticada, que existe justamente para que un NAT compartido no penalice a otros — ver el comentario de `middleware.go::RateLimitByUser`) protegiendo correctamente cuentas del fondo sintético que reciben, sostenidas ~60-80 s, más tráfico del que una cuenta real generaría (varios VU por cuenta simultáneos y en bucle, acumulando cerca de las 600 peticiones/min configuradas). Un usuario real no dispara su propio límite por minuto salvo que esté siendo abusado — que es exactamente lo que el limitador está diseñado para frenar. Con una proporción VU:cuenta más realista (100 VUs / 15 cuentas, ventana corta) la misma prueba dio 0% de 429.
- **Lo que NO se pudo verificar en esta sesión**: un fondo de 1.000 cuentas REALES (una por VU, sin compartir) habría evitado por completo el ruido de `RateLimitByUser`, pero a ~5 s por cuenta (registro + sondeo de correo + verificación + login) son ~83 min solo de `setup()` — fuera del presupuesto razonable de esta sesión. La caracterización DEFINITIVA de SC-003/SC-005 exige además el entorno desplegado (`deploy/k8s`, ≥ 2 réplicas, HPA) que esta sesión no tiene disponible; lo de aquí es evidencia de que el guion funciona correctamente y de que, incluso en un portátil con una sola réplica de cada servicio, los dos umbrales de latencia se cumplen bajo 1.000 VUs concurrentes reales.
- **Efecto colateral observado y explicado, no un defecto (retomado en T177)**: la carga sostenida generó backlog en el relay del outbox del Orquestador (`internal/outbox`, `BatchSize=100`/`Interval=2s`), que drenó solo tras varios minutos. Ver Notas de T177 para la medición completa y la distinción entre el síntoma y la causa.

- [X] T176 Verificar la tasa de inconsistencia residual < 0,1% de las operaciones distribuidas mediante inyección sistemática de fallos en cada paso de cada saga (SC-008)

**Notas de T176**:

- **Verificación auditada, no solo ejecutada**: `saga.go::run` documenta una propiedad que el motor exige de TODO paso de TODA saga y que ninguna tarea anterior había verificado sistemáticamente: si `Do` tiene éxito pero `advance` no confirma el avance (caída entre el efecto externo y su registro), la reanudación vuelve a llamar a `Do` con el MISMO estado — "de ahí que `Do` deba ser idempotente" (comentario propio del motor). T176 auditó las 5 sagas contra esta propiedad exacta, inyectando el fallo en el punto preciso que la describe (`store.advanceFail` en el avance del paso en cuestión, no un fallo genérico de participante).
- **Registro, verificación de correo y anonimización — ya cumplían, confirmado por lectura**: `auth.create_credential` usa `INSERT ... ON CONFLICT (id) DO NOTHING` (comentario propio: "así que el reintento se puede reintentar... de ahí que `Do` deba ser idempotente"); `users.create_profile` igual, verificado además por una aserción existente (`storer_postgres_test.go: ON CONFLICT (id)`); los pasos de anonimización son idempotentes por naturaleza (anonimizar algo ya anonimizado no cambia nada, documentado explícitamente en `anonymization.go`); los de verificación de correo también, y el único paso con un secreto no persistido (el token) falla RÁPIDO en vez de reintentar mal, por diseño ya documentado. Ninguno de los cuatro necesitó cambios.
- **Simulación — hallazgo real, corregido**: `simulator.compute` NO era idempotente. `ComputeRequest` no tenía clave de correlación y el repositorio insertaba sin condición, así que el reintento del motor creaba una SEGUNDA fila en `simulations` por una sola acción del usuario — inflando el historial (FR-022) y `simulations_run` de `GetActivityReport` (N-02). Corregido de punta a punta: `idempotency_key` nuevo en `ComputeRequest` (proto, T176); columna `idempotency_key` en `simulations` (migración `20260815093000`) con índice `UNIQUE` (no parcial — ver más abajo); `PgSimulations::insert` usa `ON CONFLICT (idempotency_key) DO NOTHING` y, si no devuelve fila, relee la existente por su clave en vez de fallar; `steps/simulation.go` pasa `st.SagaID` (estable entre reintentos del MISMO paso) como clave. Pruebas: `compute_con_la_misma_clave_no_duplica_el_historial` (Rust, `tests/contract.rs`) y `TestSimulationSagaRetriesTheComputeStepWithTheSameIdempotencyKey` (Go, verifica que el Orquestador manda la MISMA clave en las dos llamadas — el dedupe real lo prueba el lado Rust).
- **Calificación — segundo hallazgo real, corregido**: mismo defecto en `learning.grade_and_store_attempt`. `Learning.storeAttempt` calculaba `MAX(attempt_no)+1` e insertaba sin condición; un reintento dejaba un intento FANTASMA en el historial de un usuario que en realidad solo respondió una vez — peor que el caso del Simulador, porque corrompe directamente la garantía de FR-016 ("historial completo y ordenado") con datos visibles al usuario. Corrección simétrica a la del Simulador: `idempotency_key` en `GradeRequest` (proto); columna `idempotency_key` en `quiz_attempts` (migración `20260815094500`), índice `UNIQUE`; `QuizzesRepository.storeAttempt` con el mismo patrón `ON CONFLICT ... DO NOTHING` + relectura; `steps/grading.go` pasa `st.SagaID`. Pruebas: dos nuevas en `repositories.spec.ts` (misma clave → mismo intento, sin duplicar; sin clave → dos intentos, el comportamiento de siempre) y `TestGradingSagaRetriesTheGradingStepWithTheSameIdempotencyKey` (Go, mismo patrón que la del Simulador). Este hallazgo se descubrió DESPUÉS de corregir el del Simulador, al auditar el resto de sagas por el mismo patrón — confirmado con el usuario antes de ampliarlo (mismo alcance, mismo tratamiento) en vez de extenderlo unilateralmente.
- **Por qué el índice único NO es parcial en ninguno de los dos casos**: la primera versión usaba `UNIQUE (...) WHERE idempotency_key IS NOT NULL` (evita indexar las filas sin clave). Se simplificó a un `UNIQUE` completo por dos razones: (1) es innecesario — un `UNIQUE` estándar ya no considera dos `NULL` iguales entre sí, así que las llamadas sin clave conviven sin conflicto de todos modos, con o sin partición; (2) `pg-mem` (el doble de PostgreSQL de la suite de persistencia de Aprendizaje, T082) no soporta `ON CONFLICT (col) WHERE ... DO NOTHING` — con el índice parcial, la MISMA sentencia SQL de producción dejaba de ser ejecutable en el doble, rompiendo 7 pruebas existentes que no tienen nada que ver con T176. Verificado con un índice completo, contra PostgreSQL real (`docker exec ... psql`, inserción doble con la misma clave y con `NULL`) antes de confiar en él.
- **Verificado end-to-end contra `dev/up` real**: `cargo test`/`clippy`/`fmt` (Simulador, 45 pruebas), `go build`/`vet`/`test`/`golangci-lint` (Orquestador), `tsc`/`eslint`/`jest` (Aprendizaje, 119 pruebas) — todo limpio. Migraciones `up → down → up` verificadas en `simulator_db` y `learning_db` contra el stack real. Las cuatro specs Playwright (`us1`–`us4`) pasando juntas tras reconstruir `simulator`/`orchestrator`/`learning`. Un intento de reconstrucción parcial dejó al servicio `users` con conexiones gRPC obsoletas hacia los contenedores reemplazados (causa de una falla intermitente de `GetActivityReport` durante la verificación, resuelta con `docker compose restart users`) — artefacto operativo del flujo de reconstrucción parcial usado en esta sesión, no un defecto del código.
- [X] T177 Verificar que el 95% de las notificaciones está disponible en < 2 minutos desde el evento de origen, midiendo sobre `notification_states` (SC-007)

**Notas de T177**:

- **Hallazgo de metodología, más importante que el número en sí: medir SOLO sobre `notification_states` tiene un punto ciego**. `notification_states.created_at` se fija cuando el CONSUMIDOR de Notificación recibe el mensaje (`repo/queue.ts::enqueue`, dentro de `amqp/consumer.ts::handle`), no cuando ocurrió el evento de origen. Cualquier retraso ANTES de eso — el relay del outbox del Orquestador, la propia cola de RabbitMQ — es invisible para Notificación: su reloj ni siquiera empieza a correr hasta que el mensaje ya llegó. La carga de T175 produjo el caso real que expone el punto ciego (ver su nota final): un backlog en `event_outbox` retrasó la publicación de varios eventos `user.registered` hasta 4 minutos, y medido solo con `notification_states` el resultado habría sido 99,70% dentro de 2 minutos — bueno pero FALSO, porque no contaba el tiempo que el evento pasó atascado antes de que Notificación lo viera.
- **Medición corregida: `event_outbox.created_at` (orchestrator_db, tiempo real del evento de origen) contra `notification_states.updated_at` (notification_db, tiempo del envío)**. Es una lectura cruzada de dos bases de servicios distintos, deliberadamente fuera del código de producción — igual que `dev/demo` consulta `audit_db` directamente para verificar, esto es un guion de verificación de una sola vez y no una ruta que ningún servicio en ejecución tome (Principio III sigue intacto: ningún servicio lee la base de otro en tiempo de ejecución). Los dos conjuntos se cruzan por `event_id`, que es literalmente `event_outbox.id` — confirmado antes de confiar en el cruce.
- **Resultado (326 eventos `user.registered` con desenlace, sesión completa incluida la carga de T175)**: 98,77% dentro de 2 minutos (322/326) — **cumple SC-007** (≥ 95%) incluso contando el backlog inducido por la propia prueba de carga. Mediana y p95 reales: 5,0 s (el backlog afecta a la COLA, no al camino normal). Máximo: 598 s (~10 min) — los pocos eventos atrapados detrás del backlog de `event_outbox` mientras drenaba tras T175, exactamente el efecto colateral que esa nota anticipaba.
- **Por qué el 1,23% que incumple NO es un defecto que corregir en T177**: es la misma causa que T175 ya explica — `BatchSize=100`/`Interval=2s` del relay del outbox drenando un backlog que la propia carga sintética de 1.000 VUs generó (miles de eventos `quiz_graded`/`progress_milestone`/`simulation.executed` en minutos, un volumen muy por encima de lo que 1.000 usuarios reales generarían de forma natural). El relay se recuperó solo, sin intervención, y sin perder ningún evento (los 326 con desenlace incluyen el 100% de lo publicado — cero pérdidas, ver el conteo de T178). Ajustar `BatchSize`/`Interval` es una decisión de capacidad del entorno DESPLEGADO, no un bug de esta tarea.
- [X] T178 Verificar que el 100% de las operaciones significativas genera un registro auditable, contrastando los 11 eventos contra `audit_log` (SC-006)

**Notas de T178**:

- **10 de los 11 eventos del catálogo, verificados EN VIVO contra `audit_log`** (no solo el doble de `TestAllElevenCatalogEventsAreAcceptedAndStored` de T172, que prueba que Auditoría ACEPTA los 11 con payloads sintéticos): `user.registered`, `user.email_verified`, `auth.password_changed`, `auth.session_revoked`, `learning.article_published`, `learning.quiz_graded`, `user.progress_milestone`, `simulation.executed`, `account.anonymized` ya tenían filas reales, generadas por `dev/demo`, las pruebas end-to-end de las cuatro historias y la carga de T175. `auth.security_alert` no tenía ninguna — se disparó a propósito con 5 inicios de sesión fallidos consecutivos contra una cuenta real (la política de bloqueo, `credentials.go`, `maxFailedLoginAttempts = 5`), y apareció en `audit_log` en menos de 5 s.
- **El undécimo, `user.activity`, NO es alcanzable desde ningún punto de entrada externo — hallazgo confirmado por lectura, no introducido por esta tarea**: `ActivityDefinition` (`services/orchestrator/internal/server/steps/activity.go`) tiene su propio comentario, ya presente antes de T178: «HUECO DE CONTRATO ANOTADO: `orchestrator.proto` no tiene ningún RPC que arranque esta saga [...] hoy solo en proceso». No hay REST, gRPC ni ningún otro camino documentado para invocar `server.StartActivity` desde fuera del binario del Orquestador. Consecuencia para SC-006: un evento que el sistema desplegado NUNCA puede producir no puede violar «el 100% de las operaciones que ocurren se audita» — la operación, tal y como está cableada hoy, simplemente no ocurre. Verificado que sigue siendo así con `grep` sobre `orchestrator.proto` (sin RPC `StartActivity`) antes de darlo por bueno, en vez de asumir la nota del código.
- **Contraste de integridad, no solo de presencia**: además de que cada tipo de operación tiene registro, se comprobó que el volumen coincide con lo generado — `learning.quiz_graded`/`simulation.executed`/`user.progress_milestone` en el orden de 13.400-13.500 cada uno, consistente con las ~13.000 iteraciones de la carga sostenida de T175 más las de las cuatro historias E2E, sin discrepancia que sugiera eventos perdidos entre el outbox del Orquestador y `audit_log` (el backlog de T177 retrasó, no descartó: `event_outbox` no tiene ninguna fila sin `published_at` al cierre de esta tarea).
- [X] T179 [P] Auditar el cumplimiento del Principio VIII en todo el árbol: confirmar cero `float`/`double`/`number` para dinero o tasas en persistencia, dominio, `.proto`, eventos y JSON del borde
- [X] T180 [P] Auditar el cumplimiento del Principio IX: confirmar que ninguna capa `storer` importa tipos de transporte o de protobuf y que ninguna capa `handler` importa tipos de fila, en los 8 servicios
- [X] T181 [P] Auditar el cumplimiento del Principio III: confirmar que ningún servicio tiene credenciales de una base ajena y que `GetActivityReport` resuelve los contadores externos por gRPC; y del Principio IV/V: solo Auth y Gateway abren Redis, solo Notificación y Auditoría consumen de RabbitMQ
- [X] T182 [P] Auditar el cumplimiento del Principio X: confirmar que ningún host, puerto, credencial o URL está hardcodeado y que ningún secreto real está versionado

**Notas de T179–T182**: auditoría estática (grep + lectura dirigida, sin cambios de código — el árbol ya cumplía en las cuatro):

- **T179 (VIII, sin hallazgos)**: todos los `float64`/`f64`/`f32` reales encontrados en Go y Rust son o bien comentarios explicando POR QUÉ está prohibido (mayoría), o el caso ya documentado de `orchestrator/internal/server/{server,steps/steps}.go` — un `float64` que solo aparece al releer un payload de saga desde JSON tras una reanudación (JSON no tiene enteros), usado exclusivamente para un identificador de tipo de cálculo o un contador de intento, nunca dinero, con `//nolint:forbidigo` y comentario explicando la excepción. Rust lo prohíbe además a nivel de tipo con `#![deny(clippy::disallowed_types)]` + `clippy.toml` (bloquea `f32`/`f64` en TODO el crate, no solo en los campos financieros). TypeScript: sin `number` en campos de dinero/tasa/puntaje; los `: number` encontrados son parámetros de longitud de buffer protobuf y un índice de nivel de log, no valores financieros. `.proto`: el único entero no-string cerca de un nombre sospechoso es `points` (`users.proto`), que es genuinamente `INTEGER` en `users_db` (constraint `>= 0`) — un contador de gamificación, no una cantidad decimal. OpenAPI: sin `type: number`/`format: float|double`.
- **T180 (IX, sin hallazgos)**: ningún `storer`/repository importa `gen/fintcart/*`, `google.golang.org/grpc`, `google.golang.org/protobuf`, tipos `pb` de TS, ni (en el Simulador) `tonic`/`pb::` reales — el único `tonic::` en `repo/simulations.rs` es la macro `#[tonic::async_trait]` (utilidad de lenguaje, no un tipo de transporte). Ningún `handler`/controller de los 5 servicios Go importa `pgx`/`sqlx` directamente, y ningún `storer` importa `internal/server` (sin dependencia ascendente). La única capa `handler` que sí referencia un tipo de fila (`storer.EntryRow`) es Auditoría — `internal/handler/consumer.go` mapea el JSON del evento directamente a `EntryRow` — pero es el caso degenerado YA documentado en `plan.md` N-01: Auditoría no tiene capa `server` (consumidor puro, Principio V), así que su transporte (`consumer.go`) es necesariamente el punto de contacto directo con `storer`; no es una desviación, es la arquitectura de 2 capas prevista para este servicio.
- **T181 (III/IV/V, sin hallazgos)**: `dev/docker-compose.yaml` da a cada uno de los 7 servicios con estado un único `DB_ADDR` apuntando a SU `postgres-<svc>`/`<svc>_db`; ninguno tiene una segunda cadena de conexión hacia la base de otro. `GetActivityReport` (`users/internal/server/report.go`) resuelve `quizzes_attempted`/`simulations_run` mediante `AttemptCounter`/`SimulationCounter`, dos puertos estrechos cuyas implementaciones reales (`cmd/users/main.go`) envuelven `learningv1.NewLearningServiceClient`/`simulatorv1.NewSimulatorServiceClient` — gRPC de verdad, no lectura cruzada. `REDIS_ADDR`/construcción de cliente Redis: solo en `api-gateway/internal/ratelimit` y `auth-server/internal/storer/redis_store.go`. Consumo de RabbitMQ (`.Consume(`/`channel.consume`): solo `audit/cmd/audit/main.go` y `notification/src/amqp/consumer.ts`.
- **T182 (X, sin hallazgos)**: sin cadenas `postgres://`/`redis://`/`amqp://` hardcodeadas con host real fuera de `dev/docker-compose.yaml` (que es config de desarrollo, no código de servicio); los únicos `localhost`/`127.0.0.1` en código fuente (excluyendo `node_modules`) son dos pruebas de Aprendizaje que usan `amqp://127.0.0.1:1` deliberadamente INALCANZABLE para confirmar que `EventsPublisher` nunca lanza. `deploy/k8s/` no versiona ningún manifiesto `kind: Secret`: los Deployments referencian `secretRef: fintcart-auth-secrets` etc. por nombre, sin definir su contenido en el repo — coherente con Principio X regla 3. Ningún `.env`/`.env.*` está trackeado en git.
- [X] T183 Escribir el `README.md` raíz con el diagrama de arquitectura vigente y comandos copiables verificados
- [X] T184 Verificar que los comandos documentados en `README.md` y `specs/001-fintcart-platform/quickstart.md` coinciden exactamente con los scripts `dev/`, y añadir esa verificación al CI (Principio XII regla 5)

**Notas de T183–T184**:

- **T183**: el `README.md` raíz era una plantilla sin rellenar (placeholders `your-org`, `Project Name`, secciones vacías). Se reescribió con: resumen funcional de las cuatro historias, diagrama de arquitectura en **Mermaid** (se renderiza nativo en GitHub sin depender de exportar los `.drawio` de `docs/diagrams/`, que no se visualizan en el repositorio sin la extensión de draw.io), tabla de servicios (lenguaje/responsabilidad/base propia), los mismos cinco comandos de arranque que el paso 1 de `quickstart.md` (deliberadamente sin re-explicar health checks, herramientas de inspección ni gates — el README enlaza a `quickstart.md` en vez de duplicarlo, para no tener dos copias que diverjan), árbol del repositorio y tabla de enlaces a la documentación viva (`spec.md`, `plan.md`, `data-model.md`, `tasks.md`, `events-catalog.md`, la Constitución).
- **T184**: el job `docs-match-scripts` de `.github/workflows/ci.yml` ya existía (verificaba solo referencias obsoletas a rutas/herramientas retiradas). Se le añadió un paso nuevo que verifica la coherencia en las DOS direcciones: (a) todo `dev/<algo>` citado en `README.md`/`quickstart.md` debe existir de verdad en `dev/`; (b) todo verbo ejecutable real de `dev/` (derivado en tiempo de ejecución con `find dev -maxdepth 1 -type f -executable`, no copiado a mano en el workflow para que no se desactualice) debe aparecer citado en al menos uno de los dos documentos. Verificado localmente contra el estado actual de `dev/` (`build`, `demo`, `down`, `migrate`, `seed`, `up`) y de los dos documentos: pasa limpio.
