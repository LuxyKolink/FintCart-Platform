 ...
 *
 * COPIA EXACTA de `services/learning/src/common/decimal-str.ts`. Se duplica y no
 * se comparte porque el Frontend y Aprendizaje son proyectos npm independientes
 * (sin workspace ni paquete común) y `contracts/` versiona contratos, no código.
 * La duplicación es deliberada, pero DEBE mantenerse sincronizada: los dos
 * extremos de la frontera REST tienen que interpretar el mismo monto igual, y
 * ambos usan `decimal.js` precisamente para que no haya dos semánticas.
